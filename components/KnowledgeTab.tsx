    ...
    const PathwayCard: React.FC<{ pathway: PathwayDef }> = ({ pathway }) => {
        ...
        return (
            <div 
                ...
                className={`bg-scout-800 rounded-3xl p-6 border-2 transition-all cursor-pointer group hover:shadow-2xl relative overflow-hidden flex flex-col h-full min-h-[160px] ${pathway.color.includes('red') ? 'border-red-500/20 hover:border-red-500/50' : pathway.color.includes('blue') ? 'border-blue-500/20 hover:border-blue-500/50' : pathway.color.includes('orange') ? 'border-orange-500/20 hover:border-orange-500/50' : 'border-gray-500/20 hover:border-gray-500/50'}`}
            >
                <div className="flex items-start gap-4 mb-4">
                    <div className="p-3 bg-scout-900 rounded-2xl group-hover:scale-110 transition-transform shrink-0"><Icon size={24} /></div>
                    <h3 className="text-sm md:text-[15px] font-black text-white leading-tight uppercase tracking-tight break-words">{pathway.title}</h3>
                </div>
                <p className="text-gray-400 text-[11px] mb-6 flex-1 line-clamp-2 leading-relaxed">{pathway.shortDesc}</p>
                ...
                <div className="flex-1 p-6 overflow-y-auto bg-scout-900/30 custom-scrollbar space-y-6">
                     ...
                     {aiChatHistory.map((msg, i) => (
                         <div key={i} className={`flex flex-col animate-fade-in ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                             <div className={`max-w-[92%] p-4 rounded-2xl text-[13px] font-medium shadow-2xl ${
                                 msg.role === 'user' 
