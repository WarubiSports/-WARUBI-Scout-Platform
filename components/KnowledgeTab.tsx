...
const KnowledgeTab: React.FC<KnowledgeTabProps> = ({ user }) => {
    ...
    const handleAskAI = async (e: React.FormEvent, overrideQuestion?: string) => {
        ...
        try {
            let contextContext = `Persona: ${(user.roles || ['Scout']).join('/')}. `;
                ...
                <div className="flex-1 p-6 overflow-y-auto bg-scout-900/30 custom-scrollbar space-y-6">
                    {aiChatHistory.length === 0 && (
                        <div className="space-y-6">
                            ...
                            <div className="p-4 rounded-2xl bg-scout-accent/5 border border-scout-accent/20 text-center font-mono"><p className="text-[9px] text-scout-accent font-black uppercase tracking-wider">Context: {(user.roles || ['Scout']).join(' / ')}</p></div>
                        </div>
                    )}
